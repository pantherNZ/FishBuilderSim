...
public static partial class Utility
{
	...
	public class UI
	{
		...
		public class RepeatTransition
		{
			...
			bool active = false;
			int index = 0;
			bool transitionStartedConfirmation = false;
			...
			public void Activate()
			{
				...
				active = true;
				index = 0;
				element.RegisterCallback<TransitionStartEvent>(OnTransitionStart);
				element.RegisterCallback<TransitionEndEvent>(OnTransitionEnd);
				element.AddToClassList(ussClasses[0]);
				element.schedule.Execute(OnTransitionStartFail).StartingIn(100);
				if (deactivatedClass != "")
			...
			{
				transitionStartedConfirmation = true;
			}

			void OnTransitionStartFail()
			{
				if (transitionStartedConfirmation)
					return;
			...
			void OnTransitionEnd(TransitionEndEvent evt)
			{
				...
				if (!active)
				{
					if (deactivatedClass != "")
						element.AddToClassList(deactivatedClass);
					element.UnregisterCallback<TransitionStartEvent>(OnTransitionStart);
		...
		public class CounterLabel
		{
			...
			public CounterLabel(Label label, Label delta, float transitionTime, string unit = "", bool showPositive = true, bool max3Digits = false)
			{
			...
			{
				if (max3Digits)
					countLabel.text = Utility.ToQuantifiedString(count, 10000) + unit;
				else
