...
{
	public static uint Clamp( uint value, uint min, uint maxValue ) =>
		value < min ? min : value > maxValue ? maxValue : value;
    ...
    */
	...
	public static Vector3Int ToVector3IntXZ( this Vector2Int vec, int y = 0 ) { return new Vector3Int( vec.x, y, vec.y ); }

	public static Vector3 GetPosition( this Collider collider ) => collider.bounds.center;

	public enum ToIntRounding
	{
		Floor,
		Round,
		Ceil,
	...
	public static Vector2Int ToVector2Int( this Vector2 vec, ToIntRounding rounding = ToIntRounding.Floor )
	{
		return rounding switch
		{
			ToIntRounding.Floor => new( Mathf.FloorToInt( vec.x ), Mathf.FloorToInt( vec.y ) ),
			ToIntRounding.Round => new( Mathf.RoundToInt( vec.x ), Mathf.RoundToInt( vec.y ) ),
			ToIntRounding.Ceil => new( Mathf.CeilToInt( vec.x ), Mathf.CeilToInt( vec.y ) ),
			_ => throw new NotImplementedException(),
		};
	}

	public static Vector2Int ToVector2IntXZ( this Vector3 vec, ToIntRounding rounding = ToIntRounding.Floor )
	{
		return rounding switch
		{
			ToIntRounding.Floor => new( Mathf.FloorToInt( vec.x ), Mathf.FloorToInt( vec.z ) ),
			ToIntRounding.Round => new( Mathf.RoundToInt( vec.x ), Mathf.RoundToInt( vec.z ) ),
			ToIntRounding.Ceil => new( Mathf.CeilToInt( vec.x ), Mathf.CeilToInt( vec.z ) ),
			_ => throw new NotImplementedException(),
		};
	}

	public static Vector3Int ToVector3Int( this Vector3 vec, ToIntRounding rounding = ToIntRounding.Floor )
	{
		return rounding switch
		{
			ToIntRounding.Floor => new( Mathf.FloorToInt( vec.x ), Mathf.FloorToInt( vec.y ), Mathf.FloorToInt( vec.z ) ),
			ToIntRounding.Round => new( Mathf.RoundToInt( vec.x ), Mathf.RoundToInt( vec.y ), Mathf.RoundToInt( vec.z ) ),
			ToIntRounding.Ceil => new( Mathf.CeilToInt( vec.x ), Mathf.CeilToInt( vec.y ), Mathf.CeilToInt( vec.z ) ),
			_ => throw new NotImplementedException(),
		};
	}
	...
	public static float Rotation( this Vector2 vec ) => Angle( vec );

	public static Vector2 Rotate( this Vector2 vec, float angleDegrees )
		=> Quaternion.AngleAxis( angleDegrees, Vector3.forward ) * vec;

	public static Vector2 RotateAround( this Vector2 vec, Vector2 centre, float angleDegrees )
	{
		vec -= centre;
		vec = Quaternion.AngleAxis( angleDegrees, Vector3.forward ) * vec;
		vec += centre;
		return vec;
	}

	public static Vector3 RotateX( this Vector3 vec, float angleDegrees )
		=> Quaternion.AngleAxis( angleDegrees, Vector3.right ) * vec;

	public static Vector3 RotateY( this Vector3 vec, float angleDegrees )
		=> Quaternion.AngleAxis( angleDegrees, Vector3.up ) * vec;
	...
	public static Vector2 Clamp( this Vector2 vec, Rect rect )
	{
	...
	public static Vector2Int Clamp( this Vector2Int vec, RectInt rect )
	{
		vec.x = Mathf.Clamp( vec.x, rect.xMin, rect.xMax );
		vec.y = Mathf.Clamp( vec.y, rect.yMin, rect.yMax );
		return vec;
	...
	public static Vector2Int ClampToBorder( this Vector2Int vec, RectInt rect, int border = 0 )
	{
		if ( rect.size == Vector2Int.zero || rect.size == Vector2Int.one )
			return rect.min;

		// Clamp to the nearest edge of the rectangle expanded OUTWARD by `border`.
		// border=0 => exact perimeter; border>0 => perimeter of rect expanded by `border`.
		border = Mathf.Max( 0, border );

		int outMinX = rect.xMin - border;
		int outMaxX = rect.xMax + border - 1;
		int outMinY = rect.yMin - border;
		int outMaxY = rect.yMax + border - 1;

		// Candidate points on each edge of the expanded rectangle
		Vector2Int left = new Vector2Int( outMinX, Mathf.Clamp( vec.y, outMinY, outMaxY ) );
		Vector2Int right = new Vector2Int( outMaxX, Mathf.Clamp( vec.y, outMinY, outMaxY ) );
		Vector2Int bottom = new Vector2Int( Mathf.Clamp( vec.x, outMinX, outMaxX ), outMinY );
		Vector2Int top = new Vector2Int( Mathf.Clamp( vec.x, outMinX, outMaxX ), outMaxY );

		int dl = ( vec - left ).sqrMagnitude;
		int dr = ( vec - right ).sqrMagnitude;
		int db = ( vec - bottom ).sqrMagnitude;
		int dt = ( vec - top ).sqrMagnitude;

		int min = dl; Vector2Int best = left;
		if ( dr < min ) { min = dr; best = right; }
		if ( db < min ) { min = db; best = bottom; }
		if ( dt < min ) { /*min = dt;*/ best = top; }
	...
	public static Vector2Int RandomPosition( this RectInt rect, IRandom rng = null )
		=> new Vector2Int(
			rng.Range( rect.x, rect.x + rect.width ),
			rng.Range( rect.y, rect.y + rect.height ) );
	...
	public static float Random( this Interval bound, IRandom rng = null )
		=> ( rng ?? Utility.DefaultRng ).Range( bound.First, bound.Second );
	public static int Random( this IntervalInt bound, IRandom rng = null )
		=> ( rng ?? Utility.DefaultRng ).Range( bound.First, bound.Second + 1 );

	public static Color Random( this Gradient gradient, IRandom rng = null )
		=> gradient.Evaluate( ( rng ?? Utility.DefaultRng ).value );
	...
	public static Rect ToRect( this Bounds bound ) => new( bound.center - bound.extents, bound.size );
	public static Rect ToRectXZ( this Bounds bound ) => new( bound.center.XZ() - bound.extents.XZ(), bound.size.XZ() );
	public static Rect ToRect( this RectInt rect ) => new( rect.position, rect.size );
	public static Rect ToRectInt( this RectInt rect ) => new( rect.position, rect.size );
	public static RectInt ToRectInt( this Rect rect, ToIntRounding rounding = ToIntRounding.Floor ) => new( rect.position.ToVector2Int( rounding ), rect.size.ToVector2Int( rounding ) );

	public static bool Contains( this Rect rect, Rect other )
		=> rect.Contains( other.TopLeft() )
	...
	public static Rect Expand( this ref Rect rect, Vector2 size )
	{
		rect.size += size * 2;
		rect.center -= size;
		return rect;
	...
	public static RectInt Expand( this ref RectInt rect, Vector2Int size )
	{
		rect.size += size * 2;
		rect.position -= size;
		return rect;
	...
	public static Rect ScaleBy( this ref Rect rect, float scale )
	{
		return rect.Expand( ( scale - 1.0f ) * 0.5f * rect.size );
	}

	public static RectInt ScaleBy( this ref RectInt rect, int scale )
	{
		return rect.Expand( ( scale - 1 ) / 2 * rect.size );
	}

	public static Rect Move( this ref Rect rect, Vector2 offset )
	{
		rect.position += offset;
		return rect;
	}

	public static RectInt Move( this ref RectInt rect, Vector2Int offset )
	{
		rect.position += offset;
		return rect;
	}
	...
	public static Vector3 GetVec3FromAxisMask( this EAxisMask mask )
	{
		return new Vector3(
			mask.HasFlag( EAxisMask.X ) ? 1.0f : 0.0f,
			mask.HasFlag( EAxisMask.Y ) ? 1.0f : 0.0f,
			mask.HasFlag( EAxisMask.Z ) ? 1.0f : 0.0f
		);
	}

	public static Vector2 GetVec2FromAxisMask( this EAxisMask mask )
	{
		return new Vector2(
			mask.HasFlag( EAxisMask.X ) ? 1.0f : 0.0f,
			mask.HasFlag( EAxisMask.Y ) ? 1.0f : 0.0f
	...
	public static Vector3 ScaleBy( this Vector3 vec, Vector3 scale )
	{
		vec.Scale( scale );
		return vec;
	}

	public static Vector4 ScaleBy( this Vector4 vec, Vector4 scale )
	{
		vec.Scale( scale );
		return vec;
	}
	...
	public static string ToQuantifiedString( int number, int threshold = 1000 )
	{
		if ( number < threshold )
	...
	// Stable 64-bit hash for child identity (FNV-1a)
	public static ulong ComputeStableHash64( string s )
	{
		unchecked
		{
